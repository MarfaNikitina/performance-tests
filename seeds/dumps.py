...
def save_seeds_result(result: SeedsResult, scenario: str):
    ...
    seed_file_path = f"./dumps/{scenario}_seeds.json"
    # Сохраняем результат сидинга в файл с именем {scenario}_seeds.json
    with open(seed_file_path, 'w+', encoding="utf-8") as file:
        file.write(result.model_dump_json())
        logger.debug(f"Seeding result saved to file: {seed_file_path}")
...
def load_seeds_result(scenario: str) -> SeedsResult:
    ...
    with open(seed_file_path, 'r', encoding="utf-8") as file:
        logger.debug(f"Seeding result loaded from file: {seed_file_path}")
